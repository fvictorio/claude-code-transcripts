...
def render_edit_tool(tool_input, tool_id):
    """Render Edit tool calls with a unified-diff display."""
    file_path = tool_input.get("file_path", "Unknown file")
    old_string = tool_input.get("old_string", "")
    new_string = tool_input.get("new_string", "")
    replace_all = tool_input.get("replace_all", False)
    diff_rows = compute_edit_diff(old_string, new_string)
    return _macros.edit_tool(file_path, replace_all, tool_id, diff_rows)
...
.edit-diff { font-family: monospace; font-size: 0.85rem; margin: 4px 0; border-radius: 4px; overflow: hidden; background: #fafafa; }
.edit-line { display: flex; align-items: flex-start; min-height: 1.4em; }
.edit-marker { flex: 0 0 1.5em; padding: 2px 6px; text-align: center; font-weight: bold; user-select: none; }
.edit-line-content { flex: 1; padding: 2px 8px; white-space: pre-wrap; word-wrap: break-word; overflow-wrap: anywhere; }
.edit-line-equal { color: #555; }
